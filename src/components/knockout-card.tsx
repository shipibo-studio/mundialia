import type { FaseEliminatoriaDetail } from "@/types";

interface KnockoutCardProps {
  nombre: string;
  data: FaseEliminatoriaDetail;
}

export function KnockoutCard({ nombre, data }: KnockoutCardProps) {
  const matchCount = data.total_partidos ?? data.partidos;
  const fecha = data.fecha ?? `${data.fecha_inicio ?? ""} – ${data.fecha_fin ?? ""}`.replace(/^ – /, "").replace(/ – $/, "");
  ...
  return (
    <div className="glass-card rounded-xl p-lg relative overflow-hidden group border-white/5">
      ...
      <h3 className="typo-headline-lg mb-1">{nombre}</h3>
      <div className="flex items-center gap-xs text-text-muted mb-6">
        <span className="material-symbols-outlined text-sm">
          calendar_month
        </span>
        <span className="typo-body-md">{fecha}</span>
      </div>
      {data.sede ? (
        <div className="flex items-center gap-xs text-on-surface-variant bg-white/5 p-3 rounded-lg border border-primary/20 neon-border-cyan">
          ...
          <span className="typo-body-md">{data.sede}</span>
        </div>
      ) : data.sedes ? (
        <div className="flex items-center gap-xs text-on-surface-variant bg-white/5 p-3 rounded-lg border border-primary/20 neon-border-cyan">
          <span className="material-symbols-outlined text-sm text-primary neon-text-cyan">
          ...
          <span className="typo-body-md">{data.sedes.join(" · ")}</span>
        </div>
      ) : (
